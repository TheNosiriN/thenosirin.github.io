function ShaderToyLite(canvas, opts={}) {

    opts.alpha = opts.alpha || false;
    opts.premultipliedAlpha = opts.premultipliedAlpha || false;
    opts.depth = opts.depth || false;
    opts.stencil = opts.stencil || false;
    opts.antialias = opts.antialias || false;
    opts.preserveDrawingBuffer = opts.preserveDrawingBuffer || false;
    opts.desynchronized = opts.desynchronized || false;
    opts.vertexBufferProps = opts.vertexBufferProps || [{size:2, stride:0, offset:0}];
    opts.failIfMajorPerformanceCaveat = true;
    opts.powerPreference = opts.powerPreference || "default";
    ...
    uniform sampler3D iChannel3;             // input channel 3
    ...
    precision mediump sampler3D;
    #endif` + "\n";
    for (var i=0; i<opts.vertexBufferProps.length; ++i){
        basicVertexShaderHeader += `in ${opts.vertexBufferProps[i].size==1 ? "float" : "vec"+(opts.vertexBufferProps[i].size || 2)} ${"vertexInPosition"+(i==0 ? "" : i)};` + "\n";
    }
    ...
    var drawCount = quadVertices.length/2;
    var quadBuffer;
    ...
    var resizeFrameBuffer = (texture, framebuffer) => {
        gl.bindFramebuffer(gl.FRAMEBUFFER, framebuffer);
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texture, 0);
        gl.bindTexture(gl.TEXTURE_2D, null);
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    ...
    var compileProgram = (key, uniforms) => {
        var vert = gl.createShader(gl.VERTEX_SHADER);
        gl.shaderSource(vert, basicVertexShaderHeader+basicVertexShader);
        ...
        // user uniforms
        for (u of uniforms){
            location[key][u] = gl.getUniformLocation(program, u);
        }
        // uniform locations
        ...
        for (var i=0; i<opts.vertexBufferProps.length; ++i){
            location[key]["vertexInPosition"+i] = gl.getAttribLocation(program, "vertexInPosition"+(i==0 ? "" : i));
        }
    ...
    var setShader = (config, key) => {
        if (config) {
            if (config.source) {
                ...
                program[key] = compileProgram(key, config.uniforms || []);
                if (program[key] == null) {
        ...
        ['A', 'B', 'C', 'D', 'Image'].forEach((key) => {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                if (chkey) {
                    ...
                    gl.bindTexture(i==3 ? gl.TEXTURE_3D : gl.TEXTURE_2D, input);
                }
            }
            ...
            // uniforms
            if (onSetUniforms){ onSetUniforms(gl, location[key]); }
            ...
            for (var i=0; i<opts.vertexBufferProps.length; ++i){
                gl.vertexAttribPointer(
                    location[key]["vertexInPosition"+i],
                    opts.vertexBufferProps[i].size || 2, gl.FLOAT, false,
                    opts.vertexBufferProps[i].stride || 0, opts.vertexBufferProps[i].offset || 0
                );
                gl.enableVertexAttribArray(location[key]["vertexInPosition"+i]);
            }
    ...
    this.getVertexBufferProps = () => {
        return quadBufferProps;
    }
    ...
    this.setCommon = (source) => {
        ...
        common = source;
        // assume setCommon will always be called first
        // this is so we have the array of user uniforms when compiling the program
        // ['A', 'B', 'C', 'D', 'Image'].forEach((key) => {
        //     if (program[key]) {
    ...
    this.setOnUniforms = (callback) => {
        onSetUniforms = callback
    }
    ...
    this.resize = (width, height) => {
        ...
        ['A', 'B', 'C', 'D'].forEach((key) => {
            resizeTexture(atexture[key]);
            resizeTexture(btexture[key]);
            resizeFrameBuffer(atexture[key], aframebuf[key]);
            resizeFrameBuffer(btexture[key], bframebuf[key]);
        });
    ...
    this.bindMousemove = (event) => {
        const scale = window.devicePixelRatio;
        iMouse.x = event.offsetX * scale;
        iMouse.y = canvas.height - (event.offsetY*scale);
    };
